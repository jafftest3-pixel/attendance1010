...
_db_path_env = os.environ.get("ATTENDANCE_DB_PATH", "").strip()
_render_disk_path = os.environ.get("RENDER_DISK_PATH", "").strip()
if _db_path_env:
    DB_PATH = Path(_db_path_env)
elif _render_disk_path:
    DB_PATH = Path(_render_disk_path) / "attendance.db"
else:
...
def api_setting(key: str):
    if request.method == "OPTIONS":
        return jsonify({"status": "ok"})

    if request.method == "GET":
        return jsonify({"value": get_setting(key, "")})
